...
class WikiLanguageSelector extends WikiElement {
	...
	constructor() {
		...
		this.suggestionsPopover = this.shadowRoot.querySelector(
			".suggestions-popover",
		);
		this.languages = {};
	...
	async connectedCallback() {
		...
		if (this.languageList) {
			this.languages = this.languages.filter((lang) =>
				this.languageList.includes(lang.value),
			);
		}
		this.render();
	}
	...
	updateMenu() {
		this.suggestionsListElement.querySelectorAll("li").forEach((li) => {
			const langCode = li.getAttribute("id");
	...
	highlightActiveSuggestion() {
		let index = 0;
		for (const li of this.suggestionsListElement.children) {
			li.classList.remove("active");
