...
function getImageData(imageUrl) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            const data = context.getImageData(0, 0, img.width, img.height, { colorSpace: 'display-p3' }).data
...
function getAverageColor(data, sample = 1) {
    ...
    return [Math.round(rgb.r / amount), Math.round(rgb.g / amount), Math.round(rgb.b / amount)]
}
...
function getProminentColor(data, sample = 1, amount = 1, groupby = 20) {
    ...
    for (let i = 0; i < data.length; i += gap) {
        const rgb = [group(data[i], groupby), group(data[i + 1], groupby), group(data[i + 2], groupby)].join()
...
export { average, getColorTheme, getContinuousColor, prominent }
